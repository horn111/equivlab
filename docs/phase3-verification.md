...
- deterministic AST findings before semantic evaluation;
- a bounded semantic payload containing only failed and warning rule IDs;
- fail-closed `UNVERIFIABLE` results for fetch, hash, parse, and malformed semantic output;
...
`UNVERIFIABLE`; malformed semantic output; fetch/hash failure; leader/validator
disagreement before storage writes; duplicate audit; challenge; supersession;
latest lookup; and report hashing.

```powershell
$env:PYTEST_DISABLE_PLUGIN_AUTOLOAD='1'
python -m pytest tests/analyzer tests/contract
```

Current dependency-free result: `57 passed`; the direct-mode module is skipped
...
- combined analyzer, contract-stub, and direct-mode result: `64 passed`.
...
These are local direct-runner results with mocked web and model responses. They
...
direct-mode execution or semantic review.
